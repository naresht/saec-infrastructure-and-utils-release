 ...
 * This method prevent passing all the data you want to see in your logs in all the methods.<br>
 * Please configure log4j with the data provided by this class such as
 * <code>log4j.appender.CONSOLE.layout.ConversionPattern=%d{HH:mm:ss-SSS} [%t] %X{session_id} %p %X{login} %C.%M(%L) | %m%n</code> where
 * <code>session_id</code> and <code>login</code> are objects stored in the log context.
 */
